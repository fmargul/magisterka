...
main(int argc, char* argv[])
{
    ...
    {
        tosValues = {0xc0, 0xb8, 0x70};
    }
    else if (scenario == "8")
    {
        tosValues = {0xc0, 0xb8, 0x70, 0x28};
    }
    ...
    // Grupa 1
    ApplicationContainer sourceApplications1;
    ApplicationContainer sinkApplications1;

    uint32_t portNumber = 9;
    for (uint32_t index = 0; index < nStations; ++index)
    {
        for (uint8_t tosValue : tosValues)
        {
            ...
            onOffHelper.SetAttribute("DataRate", DataRateValue(dataRate * 1024 * 1024));
            onOffHelper.SetAttribute("PacketSize", UintegerValue(payloadSize)); // bytes
            onOffHelper.SetAttribute("Tos", UintegerValue(tosValue));
            sourceApplications1.Add(onOffHelper.Install(wifiStaNodes.Get(index)));
            PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory", sinkSocket);
            sinkApplications1.Add(packetSinkHelper.Install(wifiApNode.Get(0)));
        }
    }
    sinkApplications1.Start(Seconds(0));
    sinkApplications1.Stop(simulationTime + Seconds(1));
    sourceApplications1.Start(Seconds(1));
    sourceApplications1.Stop(simulationTime + Seconds(1));

    // Grupa 2
    ApplicationContainer sourceApplications2;
    ApplicationContainer sinkApplications2;

    for (uint32_t index = 0; index < nStations; ++index)
    {
        for (uint8_t tosValue : tosValues)
        {
            auto ipv4 = wifiApNode2.Get(0)->GetObject<Ipv4>();
            const auto address = ipv4->GetAddress(1, 0).GetLocal();
            InetSocketAddress sinkSocket(address, portNumber++);
            OnOffHelper onOffHelper("ns3::UdpSocketFactory", sinkSocket);
            onOffHelper.SetAttribute("OnTime",
            ...
            onOffHelper.SetAttribute("PacketSize", UintegerValue(payloadSize)); // bytes
            onOffHelper.SetAttribute("Tos", UintegerValue(tosValue));
            sourceApplications2.Add(onOffHelper.Install(wifiStaNodes2.Get(index)));
            PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory", sinkSocket);
            sinkApplications2.Add(packetSinkHelper.Install(wifiApNode2.Get(0)));
        }
    }
    sinkApplications2.Start(Seconds(0));
    sinkApplications2.Stop(simulationTime + Seconds(1));
    sourceApplications2.Start(Seconds(1));
    sourceApplications2.Stop(simulationTime + Seconds(1));

    // Grupa 3
    ApplicationContainer sourceApplications3;
    ApplicationContainer sinkApplications3;

    for (uint32_t index = 0; index < nStations; ++index)
    {
        for (uint8_t tosValue : tosValues)
        {
            auto ipv4 = wifiApNode3.Get(0)->GetObject<Ipv4>();
            const auto address = ipv4->GetAddress(1, 0).GetLocal();
            InetSocketAddress sinkSocket(address, portNumber++);
            OnOffHelper onOffHelper("ns3::UdpSocketFactory", sinkSocket);
            onOffHelper.SetAttribute("OnTime",
            ...
            onOffHelper.SetAttribute("PacketSize", UintegerValue(payloadSize)); // bytes
            onOffHelper.SetAttribute("Tos", UintegerValue(tosValue));
            sourceApplications3.Add(onOffHelper.Install(wifiStaNodes3.Get(index)));
            PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory", sinkSocket);
            sinkApplications3.Add(packetSinkHelper.Install(wifiApNode3.Get(0)));
        }
    }
    sinkApplications3.Start(Seconds(0));
    sinkApplications3.Stop(simulationTime + Seconds(1));
    sourceApplications3.Start(Seconds(1));
    sourceApplications3.Stop(simulationTime + Seconds(1));

    // Grupa 4
    ApplicationContainer sourceApplications4;
    ApplicationContainer sinkApplications4;

    for (uint32_t index = 0; index < nStations; ++index)
    {
        for (uint8_t tosValue : tosValues)
        {
            auto ipv4 = wifiApNode4.Get(0)->GetObject<Ipv4>();
            const auto address = ipv4->GetAddress(1, 0).GetLocal();
            InetSocketAddress sinkSocket(address, portNumber++);
            OnOffHelper onOffHelper("ns3::UdpSocketFactory", sinkSocket);
            onOffHelper.SetAttribute("OnTime",
            ...
            onOffHelper.SetAttribute("PacketSize", UintegerValue(payloadSize)); // bytes
            onOffHelper.SetAttribute("Tos", UintegerValue(tosValue));
            sourceApplications4.Add(onOffHelper.Install(wifiStaNodes4.Get(index)));
            PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory", sinkSocket);
            sinkApplications4.Add(packetSinkHelper.Install(wifiApNode4.Get(0)));
        }
    }
    sinkApplications4.Start(Seconds(0));
    sinkApplications4.Stop(simulationTime + Seconds(1));
    sourceApplications4.Start(Seconds(1));
    sourceApplications4.Stop(simulationTime + Seconds(1));
    ...
    if (fileExists(csvPath)) {
        ...
        myfile.open (csvPath, std::ios::app);  
        myfile << "rngRun,flow,nStations,distance,linksNumber,loss,jitter,latency,throughput,rxBytes,txBytes,rxPackets,txPackets,timeFirstRxPacket,timeFirstTxPacket,timeLastRxPacket,timeLastTxPacket,jitterSum,delaySum,rateAdaptationManager,scenario,MLOn,paddingDelayUsec,transitionDelayUsec,channelSwitchDelay" << std::endl;
    }
    for (auto flow : stats)
    {
        ...
        myfile << RngSeedManager::GetRun () << ',' << flow.first << ',' << nStations << ',' << distance << ',' << nLinks << ',' << lossRate << ',' << avgJitter<< ','<< avgLatency<< ',' << throughput<< ',' << rxBytes << ',' << txBytes << ',' << rxPackets << ',' << txPackets << ',' << timeFirstRxPacket << ',' << timeFirstTxPacket << ',' << timeLastRxPacket << ',' << timeLastTxPacket << ',' << jitterSum << ',' << delaySum << ',' << rateAdaptationManager << ',' << scenario << ',' << MLOn << ',' << paddingDelayUsec << ',' << transitionDelayUsec << ',' << channelSwitchDelayUsec << std::endl;
